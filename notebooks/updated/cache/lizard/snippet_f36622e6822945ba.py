def run(path, code=None, params=None, ignore=None, select=None, **meta):
    ...
    return [{'lnum': block.lineno, 'col': block.col_offset, 'type': 'R',
        'number': 'R709', 'text': 'R701: %s is too complex %d' % (block.
        name, block.complexity)} for block in visitor.blocks if block.
