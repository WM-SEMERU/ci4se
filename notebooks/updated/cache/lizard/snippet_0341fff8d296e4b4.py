def zone_exists(zone, region=None, key=None, keyid=None, profile=None,
    retry_on_rate_limit=None, rate_limit_retries=None, retry_on_errors=True,
    error_retries=5):
    if region is None:
        region = 'universal'
    ...
    if retry_on_rate_limit or rate_limit_retries is not None:
        salt.utils.versions.warn_until('Neon',
            "The 'retry_on_rate_limit' and 'rate_limit_retries' arguments have been deprecated in favor of 'retry_on_errors' and 'error_retries' respectively. Their functionality will be removed, as such, their usage is no longer required."
            )
        ...
        try:
            ...
            if retry_on_errors:
                if 'Throttling' == e.code:
                    ...
                    log.debug(
                        'The request was rejected by AWS API.                              Route 53 was still processing a prior request'
                        )
                time.sleep(3)
                error_retries -= 1
                continue
