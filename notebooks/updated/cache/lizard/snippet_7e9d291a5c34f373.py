def calculate_crop_output_shapes(operator):
    check_input_and_output_numbers(operator, input_count_range=[1, 2],
    ...
    if len(operator.inputs) == 1:
        if len(params.cropAmounts.borderAmounts) > 0:
            output_shape[2] -= params.cropAmounts.borderAmounts[0
                ].startEdgeSize
            output_shape[2] -= params.cropAmounts.borderAmounts[0].endEdgeSize
            output_shape[3] -= params.cropAmounts.borderAmounts[1
                ].startEdgeSize
