    ...
    for tarinf in tf_stream:
        try:
            ...
            if v.attrib['version'] != '2.0':
                logger.debug('Packet: {} is not VO-schema version 2.0.'.
                    format(tarinf.name))
            n_parsed += 1
        except:
        ...
        try:
            ...
            if check_for_duplicates:
                if ivorn_present(session, new_row.ivorn):
                    logger.debug('Ignoring duplicate ivorn: {} in file {}'.
                        format(new_row.ivorn, tarinf.name))
                    continue
            session.add(new_row)
            n_loaded += 1
        except:
            logger.exception(
                'Error converting file {} to database row, skipping'.format
                (tarinf.name))
            continue
        if n_loaded % pkts_per_commit == 0:
            session.commit()
    session.commit()
    logger.info('Successfully parsed {} packets, of which loaded {}.'.
