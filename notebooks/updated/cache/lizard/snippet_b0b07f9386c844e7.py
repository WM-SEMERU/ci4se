def run_count(bam_file, dexseq_gff, stranded, out_file, data):
    ...
    if not dexseq_gff:
        ...
        logger.info(
            '%s was not found, so exon-level counting is being skipped.' %
            dexseq_gff)
        return None
    ...
    if file_exists(out_file):
        return out_file
    cmd = (
        '{bcbio_python} {dexseq_count} -f bam -r {sort_flag} -p {paired_flag} -s {strand_flag} {dexseq_gff} {bam_file} {tx_out_file}'
        )
