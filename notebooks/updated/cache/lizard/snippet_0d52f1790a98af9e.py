def run(self):
    ...
    try:
        self._bitstrings = self.connection._qvm_run(quil_program=
            quil_program, classical_addresses=classical_addresses, trials=
            trials, measurement_noise=self.measurement_noise, gate_noise=
            self.gate_noise, random_seed=self.random_seed)['ro']
    except KeyError:
        warnings.warn(
            "You are running a QVM program with no MEASURE instructions. The result of this program will always be an empty array. Are you sure you didn't mean to measure some of your qubits?"
            )
