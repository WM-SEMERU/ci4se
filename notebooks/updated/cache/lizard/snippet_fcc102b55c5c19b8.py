def parse(root, describe_command=DEFAULT_DESCRIBE, pre_parse=
    warn_on_shallow, config=None):
    if not config:
    ...
    if ret:
        ...
        return meta('0.0', distance=wd.count_all_nodes(), node='g' +
            rev_node, dirty=dirty, branch=wd.get_branch(), config=config)
    else:
        ...
        if number:
            ...
            return meta(tag, config=config, node=node, dirty=dirty, branch=
                branch)